...
2. Ajoutez les **Environment Variables** (Project → Settings → Environment Variables) : `NEWSAPI_KEY`, `ADMIN_TOKEN`, `NEWSAPI_CACHE`, `NEWSAPI_CACHE_TTL`, `PREFER_DEPARTEMENT` (optionnel — mettez `true` pour favoriser les résultats au niveau départemental, ex: **Haute‑Vienne / 87**). Optionnel: `NEWSAPI_LOOKBACK_DAYS` (default: 30) pour limiter la fenêtre de recherche NewsAPI si votre plan la restreint. Note: vous pouvez activer un **mode strict** par défaut en définissant `STRICT_DEFAULT=true` — le filtre strict exige que les articles contiennent une référence à **Limoges** (ou la source/url indique Limoges) **et** un mot lié aux élections (ex: "municipales", "élection").
