...
const REMOVED_TAGS_FILE = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(DATA_DIR, 'removedTags.json');
const FEEDS_FILE = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(DATA_DIR, 'feeds.json');
...
const DEFAULT_FEEDS = [
    ...
    {
        url: 'https://news.google.com/rss/search?q=site:lepopulaire.fr+Limoges+municipales+2026&hl=fr&gl=FR&ceid=FR:fr',
        name: 'Google News (Le Populaire - Limoges)'
    },
    {
        url: 'https://news.google.com/rss/search?q=site:lepopulaire.fr+Haute-Vienne+municipales+2026&hl=fr&gl=FR&ceid=FR:fr',
        name: 'Google News (Le Populaire - Haute-Vienne)'
    },
    {
        url: 'https://news.google.com/rss/search?q=site:actu.fr+Limoges+municipales+2026&hl=fr&gl=FR&ceid=FR:fr',
        name: 'Google News (Actu.fr - Limoges)'
    },
    // Regional feeds
    {
    ...
    {
        url: 'https://news.google.com/rss/search?q=site:francebleu.fr+Limoges+municipales+2026&hl=fr&gl=FR&ceid=FR:fr',
        name: 'Google News (France Bleu - Limoges)'
    },
    // Google News searches for specific electoral coverage
    {
        url: 'https://news.google.com/rss/search?q=élections+municipales+Limoges+2026&hl=fr&gl=FR&ceid=FR:fr',
        name: 'Google News (élections municipales)'
    },
    {
        url: 'https://news.google.com/rss/search?q=municipales+Limoges+candidats&hl=fr&gl=FR&ceid=FR:fr',
        name: 'Google News (candidats Limoges)'
    }
];
async function loadFeeds() {
    try {
        ...
        const raw = await __TURBOPACK__imported__module__$5b$externals$5d2f$fs$2f$promises__$5b$external$5d$__$28$fs$2f$promises$2c$__cjs$29$__["default"].readFile(FEEDS_FILE, 'utf8');
...
async function readRemovedTags() {
    try {
        ...
        const raw = await __TURBOPACK__imported__module__$5b$externals$5d2f$fs$2f$promises__$5b$external$5d$__$28$fs$2f$promises$2c$__cjs$29$__["default"].readFile(REMOVED_TAGS_FILE, 'utf8');
...
async function saveFeeds() {
    try {
        ...
        await __TURBOPACK__imported__module__$5b$externals$5d2f$fs$2f$promises__$5b$external$5d$__$28$fs$2f$promises$2c$__cjs$29$__["default"].writeFile(FEEDS_FILE, JSON.stringify(RSS_FEEDS, null, 2), 'utf8');
...
if (typeof setInterval !== 'undefined') {
    if (!/*TURBOPACK member replacement*/ __turbopack_context__.g.__NEWS_CLEANUP_INTERVAL_SET) {
        /*TURBOPACK member replacement*/ __turbopack_context__.g.__NEWS_CLEANUP_INTERVAL_SET = true;
...
function sameText(a, b) {
    const norm = (s)=>(normalizeTextSimple(s) || '').replace(/\s+/g, ' ').trim();
    ...
    try {
        ...
        if (alts.length > 0) {
            ...
            for (const alt of alts){
                if (!RSS_FEEDS.some((f)=>f.url === alt) && !blacklistedFeeds.has(alt)) {
                    RSS_FEEDS.push({
                        url: alt,
                        name: `Alternative for ${name || host}`
                    });
        ...
        try {
            ...
            if (!RSS_FEEDS.some((f)=>f.url === genericAlt) && !blacklistedFeeds.has(genericAlt)) {
                RSS_FEEDS.push({
                    url: genericAlt,
                    name: `Generic alternative for ${host}`
                });
...
function isStrictMatch(matches, context = {}) {
    ...
    const fullText = normalizeTextSimple((context.title || '') + ' ' + (context.description || '') + ' ' + (context.source || '') + ' ' + (context.url || ''));
    ...
    const titleAndDesc = normalizeTextSimple((context.title || '') + ' ' + (context.description || ''));
    ...
    if (norm.length === 1 && (norm[0] === 'candidat' || norm[0] === 'candidats' || onlyElectionTags.includes(norm[0]))) return false;
    return hasRegion;
}
async function fetchRssFeeds(limit = 300, debug = false) {
    console.log('[fetchRssFeeds] RSS_FEEDS count:', RSS_FEEDS.length);
    if (RSS_FEEDS.length === 0) {
        console.warn('[fetchRssFeeds] RSS_FEEDS is empty; attempting to load feeds from file synchronously');
        try {
            const loaded = await loadFeeds();
            if (Array.isArray(loaded) && loaded.length) {
                RSS_FEEDS.push(...loaded);
                console.log('[fetchRssFeeds] Loaded feeds from file:', RSS_FEEDS.length);
            } else {
                console.warn('[fetchRssFeeds] loadFeeds returned no feeds; using DEFAULT_FEEDS');
                RSS_FEEDS.push(...DEFAULT_FEEDS);
            }
        } catch (e) {
            console.warn('[fetchRssFeeds] Failed to load feeds from file:', e && e.message ? e.message : e);
            RSS_FEEDS.push(...DEFAULT_FEEDS);
        }
    }
    const items = [];
    ...
    await Promise.all(RSS_FEEDS.map(async (f)=>{
        ...
        try {
            const resp = await (0, __TURBOPACK__imported__module__$5b$externals$5d2f$node$2d$fetch__$5b$external$5d$__$28$node$2d$fetch$2c$__esm_import$2c$__$5b$project$5d2f$node_modules$2f$node$2d$fetch$29$__["default"])(f.url, {
                headers: {
            ...
            const filtered = feedItems.map((it)=>{
                ...
                const scored = Object.assign({}, it, {
                    matches,
                    primaryMatch,
                    score: scoreFor(Object.assign({}, it, {
                        matches
                    }))
                });
                return scored;
...
async function fetchAndParseFeed(url, limit = 5) {
    try {
        const resp = await (0, __TURBOPACK__imported__module__$5b$externals$5d2f$node$2d$fetch__$5b$external$5d$__$28$node$2d$fetch$2c$__esm_import$2c$__$5b$project$5d2f$node_modules$2f$node$2d$fetch$29$__["default"])(url, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (X11; Linux) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120 Safari/537.36',
                'Accept': 'application/rss+xml, application/xml, text/xml, */*'
            }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b7bf127b._.js.map